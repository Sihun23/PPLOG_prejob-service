...
public class PrejobController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PrejobResponse.JobDetail>>> getMemberPrejobs(@RequestHeader(name = "memberId") String memberId) {
        Long convertedMemberId = Long.valueOf(memberId);
        List<PrejobResponse.JobDetail> response = prejobService.getPrejobsForMember(convertedMemberId);
        return ResponseEntity.ok(ApiResponse.onSuccess(response));
    }

    @Operation(summary = "API 명세서 v0.4 line 23", description = "관심직군 등록/수정")
    @PostMapping
    public ResponseEntity<ApiResponse<Object>> savePrejobs(@RequestHeader(name = "memberId") String memberId, @Valid @RequestBody PrejobRequest prejobRequest) {
        Long convertedMemberId = Long.valueOf(memberId);
        prejobService.savePrejobs(convertedMemberId, prejobRequest);
